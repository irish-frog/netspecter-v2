...
class WebSecurityTests(unittest.TestCase):
    ...
    def test_fresh_install_interval_matches_live_collection_intent(self):
        ...
        self.assertEqual(24, self.module.DEFAULT_CONFIG["raw_dns_retention_hours"])
        self.assertEqual(24, example["raw_dns_retention_hours"])
        ...
        self.assertNotIn("api_keys", self.module.DEFAULT_CONFIG)
        self.assertNotIn("api_keys", example)
    ...
    def test_web_service_uses_gunicorn_wsgi_entrypoint(self):
        ...
        self.assertIn("ethtool", installer)
        self.assertIn("suricata -T -c /etc/suricata/suricata.yaml", installer)
        self.assertIn("install_suricata_safety_override", installer)
        self.assertIn("install_nic_offload_service", installer)
        self.assertTrue((SOURCE_DIR / "scripts" / "configure-ids-interfaces.sh").exists())
        self.assertTrue((SOURCE_DIR / "systemd" / "netspecter-nic-offload.service").exists())
        self.assertIn("guard_suricata_restart_loop", installer)
        self.assertIn("configure_suricata_interface", installer)
        self.assertIn("NetSpecter placeholder. suricata-update will replace this", installer)
        ...
        self.assertIn("ensure_https_certificate", installer)
        self.assertIn("detect_primary_ip", installer)
        self.assertIn('subjectAltName=${san}', installer)
        self.assertIn("IP:127.0.0.1,DNS:localhost,DNS:netspecter.local", installer)
        self.assertIn('chmod 640 "$key_path"', installer)
        self.assertIn("ensure_secret_files", installer)
        self.assertIn('"$CONFIG_DIR/session.key"', installer)
        self.assertIn('"$CONFIG_DIR/secret.key"', installer)
        self.assertIn("NETSPECTER_WEB_WORKERS", gunicorn_config)
        self.assertIn("multiprocessing.cpu_count()", gunicorn_config)
        self.assertNotIn("workers = 2", gunicorn_config)
        self.assertNotIn("CPUQuota=200%", (SOURCE_DIR / "systemd" / "netspecter-collector.service").read_text())
        ...
        self.assertIn('cp systemd/netspecter-db-maintenance.service "$SERVICE_DIR/netspecter-db-maintenance.service"', installer)
        self.assertIn('cp systemd/netspecter-db-maintenance.timer "$SERVICE_DIR/netspecter-db-maintenance.timer"', installer)
        self.assertIn('cp systemd/netspecter-update.service "$SERVICE_DIR/netspecter-update.service"', installer)
        self.assertIn('cp systemd/netspecter-update.path "$SERVICE_DIR/netspecter-update.path"', installer)
        self.assertIn("netspecter-db-maintenance.timer", installer)
        self.assertIn("netspecter-update.path", installer)
        self.assertTrue((SOURCE_DIR / "scripts" / "daily-db-maintenance.sh").exists())
        self.assertTrue((SOURCE_DIR / "scripts" / "run-ui-update.sh").exists())
        self.assertTrue((SOURCE_DIR / "systemd" / "netspecter-update.service").exists())
        self.assertTrue((SOURCE_DIR / "systemd" / "netspecter-update.path").exists())
        maintenance = (SOURCE_DIR / "scripts" / "daily-db-maintenance.sh").read_text()
        self.assertIn("prune_suricata_raw_logs", maintenance)
        self.assertIn("prune_dns_history", maintenance)
        self.assertIn("VACUUM", maintenance)
        self.assertTrue((SOURCE_DIR / "systemd" / "netspecter-db-maintenance.service").exists())
        maintenance_service = (SOURCE_DIR / "systemd" / "netspecter-db-maintenance.service").read_text()
        self.assertIn("ExecStartPre=-/bin/systemctl stop netspecter-collector.service", maintenance_service)
        self.assertIn("ExecStartPost=-/bin/systemctl start netspecter-collector.service", maintenance_service)
        self.assertTrue((SOURCE_DIR / "systemd" / "netspecter-db-maintenance.timer").exists())
    ...
    def test_speed_test_is_manual_post_action_and_installed(self):
        ...
        self.assertIn('"speedtest-cli", "/usr/bin/speedtest-cli", "/usr/local/bin/speedtest-cli", "speedtest", "/usr/bin/speedtest"', source)
        self.assertIn('"--accept-license", "--accept-gdpr", "--format=json"', source)
        self.assertIn('"--json"', source)
        self.assertIn('float(download.get("bandwidth")) * 8 / 1000000.0', source)
        self.assertIn('if "librespeed" in candidate.lower():', source)
        ...
        self.assertIn('"$INSTALL_DIR/venv/bin/pip" install speedtest-cli', installer)
        self.assertIn("No supported speed test client is installed", source)
        self.assertNotIn("librespeed-cli", source)
        self.assertNotIn("librespeed-cli", installer)
        self.assertNotIn("ookla/speedtest-cli/script.deb.sh", installer)
    ...
    def test_optional_unifi_discovery_and_scheduled_speed_history_ship_disabled(self):
        ...
        self.assertEqual([], example["ids_banned_domains"])
    ...
    def test_update_checker_prefers_installed_checkout_and_origin_branch(self):
        ...
        self.assertIn('safe.directory={source_root}', source)
    ...
    def test_suricata_ids_alerts_page_has_filtered_notification_actions(self):
        ...
        self.assertNotIn("Only show alerts from source IPs not already known in Devices", source)
        self.assertNotIn("<h2>Alert Display Filters</h2>", source)
        self.assertIn("ids-management-grid", source)
        self.assertNotIn("hidden alerts remain in Suricata logs", source)
        ...
        self.assertIn('value="add_destination_exception"', source)
        ...
        self.assertIn('value="bulk_destination_exceptions"', source)
        self.assertIn('value="bulk_rule_exceptions"', source)
        self.assertIn('value="bulk_ban_sources"', source)
        self.assertIn('value="bulk_ban_destinations"', source)
        self.assertIn("Selected Actions", source)
        self.assertIn('value="clear_bans"', source)
        self.assertIn("Related FQDNs are blocked in AdGuard", source)
        self.assertIn('value="clear_exceptions"', source)
        self.assertIn("def ids_reopen_ignored_alerts():", source)
        self.assertIn("ids-exception-table", source)
        ...
        self.assertIn('action in {"ban_fqdn", "unban_fqdn"}', source)
        self.assertIn("def ids_apply_domain_bans_for_ips(config, endpoint_ips):", source)
        self.assertIn("def ids_clear_domain_bans(config):", source)
        self.assertIn("Related FQDN", source)
        self.assertIn("Firewall Ban List", source)
        self.assertNotIn('/device/block/', source)
        self.assertIn('"ids_banned_ips": [],', collector)
        self.assertIn('"ids_banned_domains": [],', collector)
        ...
        self.assertIn('destination_ip = str(exception.get("destination_ip") or "").strip()', collector)
        self.assertIn('if ids_alert_is_excepted(config, alert):', collector)
        self.assertIn("is_default_suppressed_signature", collector)
