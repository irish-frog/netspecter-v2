...
def ids_exception_rules(config):
    rules = config.get("ids_exceptions", [])
    return rules if isinstance(rules, list) else []


def ids_alert_matches_exception(alert, rule):
    ...
    if request.method == "POST":
        ...
        if action in {"investigate_alert", "acknowledge_alert", "close_alert", "suppress_alert", "reopen_alert", "delete_alert"}:
            ...
            if action in {"add_source_exception", "add_signature_exception"} and not valid_ipv4_ip(source_ip):
                ...
                restart_collector_service()
                return redirect("/ids-alerts?saved=exception")
        elif action == "remove_exception":
            try:
                index = int(request.form.get("exception_index", "-1"))
            except ValueError:
                index = -1
            rules = ids_exception_rules(c)
...
"""
    ...
    if request.args.get("saved") == "exception":
    ...
    sort_options = "".join(
        f'<option value="{value}"{" selected" if ids_filters["sort"] == value else ""}>{label}</option>'
...
<tr>
  ...
  <td><form class="ids-action" method="post">{csrf_input()}<input type="hidden" name="endpoint_ip" value="{h(banned_ip)}"><button type="submit" name="action" value="unban_ip">Remove Ban</button></form></td>
    ...
    exception_rows = ""
    for idx, rule in enumerate(exception_rules):
        if not isinstance(rule, dict):
            continue
        source_ip = str(rule.get("source_ip") or "").strip()
        destination_ip = str(rule.get("destination_ip") or "").strip()
...
def settings():
    ...
    setting_help = {
        ...
        "netbios_discovery_enabled": "Use background NetBIOS lookup to learn Windows PC names. Runs in the collector, never during dashboard rendering.",
    ...
    setting_labels = {
        ...
        "netbios_discovery_enabled": "NetBIOS PC Name Discovery",
        "netbios_discovery_interval_seconds": "NetBIOS Discovery Interval Seconds",
        "netbios_discovery_batch_size": "NetBIOS Devices Per Pass",
        "mdns_discovery_enabled": "mDNS / Avahi Name Discovery",
        "snmp_name_discovery_enabled": "SNMP sysName Discovery",
        "ssdp_discovery_enabled": "SSDP / UPnP Name Discovery",
        "vendor_fallback_names_enabled": "Vendor Fallback Names",
